...
public class DataManager {
  ...
  public void checkAndFixConfigKeys() {
    ...
    if (!cfg.contains("messages.drop-text-message")) {
      cfg.set(
        "messages.drop-text-message",
        "&3&lConfirm drop &r&9by dropping item again!"
      );
      modified = true;
    }
    if (!cfg.contains("messages.drop-actionbar-message")) {
      cfg.set(
        "messages.drop-actionbar-message",
        "&3&lConfirm drop &r&9by dropping item again!"
      );
      modified = true;
    }
    ...
    if (modified) {
      saveConfig();
      JavaPlugin
        .getPlugin(SafeDrop.class)
        .getLogger()
        .info(
          "[SafeDrop] config.yml was missing some values! Adding them now.."
        );
    }
  }
